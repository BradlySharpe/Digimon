...
class Dbase {
	...
	private function connect() {
		...
		if (!$this->_conndb) {
			$this->game->sendError("Database connection failed: " . mysql_error());
		} else {
			$_select = mysql_select_db($this->_name, $this->_conndb);
			if (!$_select) {
				$this->game->sendError("Database selection failed:<br />" . mysql_error());
			}
		}
	...
	public function close() {
		if (!mysql_close($this->_conndb)) {
			$this->game->sendError("Closing connection failed.");
		}
	}
